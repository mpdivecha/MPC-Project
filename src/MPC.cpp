...
double ref_v = 46 * 0.44704;

// The solver takes all the state variables and actuator
// variables in a singular vector. Thus, we should establish when
// one variable starts and another ends to make our lives easier
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  FG_eval(Eigen::VectorXd coeffs, vector<double> cost_multipliers) { 
      this->coeffs = coeffs;
      this->cost_multipliers = cost_multipliers;
  }
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 2; t++)
    {
        fg[0] += cost_multipliers[5]*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
        fg[0] += cost_multipliers[6]*CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);
    }

    // Setup constraints

    // Initial constraints
    // Need to add 1 since fg[0] is reserved for cost
...
MPC::MPC() {
    for (int i = 0; i < 7; i++)
        cost_multipliers.push_back(1.0);
}

MPC::MPC(vector<double> c_mults) {
    for (int i = 0; i < c_mults.size(); i++)
        cost_multipliers.push_back(c_mults[i]);
}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  FG_eval fg_eval(coeffs, cost_multipliers);
